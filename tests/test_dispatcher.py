...
def test_execute_tool_calls_known_tools(load_module) -> None:
    ...
    tools.TOOL_REGISTRY["bash"].execute = tools.async_tool(
        lambda params: tools.run_bash(params.command)
    )
...
def test_task_tool_uses_configured_runner(load_module) -> None:
    ...
    tools.configure_task_runner(task_runner)
...
def test_task_tool_runs_configured_runner_concurrently(load_module) -> None:
    tools = load_module("tools", "tools.py")
    active = 0
    max_active = 0
