...
- `session.py`: `AgentSession` dataclass and stop-gate implementations; one `AgentSession` owns every dependency a single agent run needs (workspace, todo, registry, permission service, trace context, stop gate)
...
- `tests/`: behavior-focused pytest coverage for the loop, tools, permissions, paths, message protocol, and session isolation
...
- A session's workspace, todo, tool registry, permission service, trace context, and stop gate live on one `AgentSession` instance, never on module-level singletons.
...
1. `./.venv/bin/python -m py_compile main.py tools.py prompts.py message_utils.py session.py workspace.py`
